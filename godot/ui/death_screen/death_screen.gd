...
func _on_begin_anew_confirmed() -> void:
	# Null refs before disk op, change scene after — subscribers never observe a
	# populated dead world during the swap. The new Main supplies the real
	# MapPanel rect via its own bootstrap(); regen does not happen here.

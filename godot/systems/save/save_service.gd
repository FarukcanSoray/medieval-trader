...
func wipe_and_regenerate() -> void:
	_generate_fresh()
	await write_now()
	_dirty = false

func _generate_fresh() -> void:
	var world_seed: int = int(Time.get_unix_time_from_system())
	...
	t.gold = 100
	t.age_ticks = 0
	# §5 worked example anchors the player at the cheap-wool node.
	t.location_node_id = "hillfarm"
	t.travel = null
